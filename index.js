...
if (fs.existsSync(SERVICE_KEY_PATH)) {
    try {
        const serviceAccount = JSON.parse(fs.readFileSync(SERVICE_KEY_PATH, "utf-8"));
...
async function executeTool(name, args) {
    ...
    try {
        if (name === "run_shell") {
            ...
            if (out)
                console.log(`Result: ${out.slice(0, 100)}...`);
            if (err)
        ...
        if (name === "find_git_repos") {
            ...
            function search(dir, depth) {
                if (depth > 5)
                    return; // Limit depth to avoid taking forever
                if (foundRepos.length >= 20)
                    ...
                    for (const entry of entries) {
                        if (entry.isDirectory()) {
                            if (entry.name.startsWith("."))
                                continue; // Skip hidden dirs (except .git check above)
