...
export const api: ElectronAPI = isElectron
    ...
    : {
          ...
          runHealthCheck: async (_force?: boolean) => ({
          ...
          aiExplainTool: async () => ({ success: true, explanation: "# Mock explanation" }),
          readToolSource: async () => ({ success: true, content: "// Mock source code" }),
          aiGenerateCustomTool: async () => ({ success: true, tool: { name: "generated_tool", description: "Generated tool description", parameters: [], code: "" } }),
          aiModifyBuiltinTool: async () => ({ success: true }),
          getGitDiff: async () => ({ success: true, diff: "" }),
