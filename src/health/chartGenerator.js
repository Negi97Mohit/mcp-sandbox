...
import { getStatsHistory, getToolUsageBreakdown } from "./statsTracker.js";
 ...
 */
export async function generateUsageChart(days = 7) {
 ...
 */
export async function generateResponseTimeChart(days = 7) {
    const stats = await getStatsHistory(days);
 ...
 */
export async function generateToolPieChart(days = 7) {
 ...
 */
export async function generateHealthTimelineChart(days = 7) {
        ...
        options: {
            ...
            scales: {
                y: {
                    ...
                    ticks: {
                        callback: (value) => {
                            const labels = { 1: "Critical", 2: "Degraded", 3: "Healthy" };
                            return labels[value] || "";
                        }
                    }
                }
 ...
 */
export async function generateErrorsChart(days = 7) {
    const stats = await getStatsHistory(days);
    const labels = stats.map(s => s.date.slice(5));
 ...
 */
export async function generateStatsEmbed(days = 7) {
    const stats = await getStatsHistory(days);
    // Calculate totals
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: "📨 Messages", value: `${totalMessages}`, inline: true }, { name: "🔧 Tool Calls", value: `${totalToolCalls}`, inline: true }, { name: "❌ Errors", value: `${totalErrors}`, inline: true }, { name: "⏱️ Avg Response", value: `${avgResponseTime}ms`, inline: true })
        .setTimestamp()
 ...
 */
export async function generateChartEmbeds(days = 7) {
    const embeds = [];
    // Usage Chart
