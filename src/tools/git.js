...
export async function handleGitTool(name, args, context) {
    ...
    try {
        if (name === "git_clone") {
            ...
            if (context)
                await context.sendLog(`📥 *Cloning repository: ${args.repo_url} into ${targetDir}...*`);
        ...
        if (name === "git_push") {
            ...
            if (!branch)
                return { error: "No active branch found to push." };
            if (context)
                await context.sendLog(`📤 *Pushing local changes to ${remote}/${branch}...*`);
        ...
        if (name === "git_pull") {
            ...
            const branch = args.branch || currentBranch;
            if (!branch)
                return { error: "No active branch found to pull." };
            if (context)
                await context.sendLog(`📥 *Pulling changes from ${remote}/${branch}...*`);
