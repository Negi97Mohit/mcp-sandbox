...
export async function handleProjectTool(name: string, args: any, context?: ToolContext) {
    ...
    try {
        if (name === "project_init") {
            ...
            if (context) await context.sendLog(`🚀 *Initializing template "${args.template}" in: ${targetDir}...*`);
        ...
        if (name === "docker_container_manage") {
            ...
            if (action === "build") {
                if (!args.image_name) return { error: "Image name (tag) is required to build a Docker image." };
                ...
                if (context) await context.sendLog(`🐳 *Building docker image ${args.image_name} from ${dockerfilePath}...*`);
                return handleShellCommand({ command: fullCmd }, context);
            }

            if (action === "run") {
                if (!args.image_name) return { error: "Image name is required to run a container." };
                if (!args.container_name) return { error: "Container name is required to run an instance." };
                ...
                if (context) await context.sendLog(`🐳 *Launching docker container ${args.container_name} (${args.image_name})...*`);
                return handleShellCommand({ command: cmd }, context);
            }

            if (action === "stop") {
                if (!args.container_name) return { error: "Container name or ID is required to stop/delete." };
                
                if (context) await context.sendLog(`🐳 *Stopping and removing container: ${args.container_name}...*`);
                const cmd = `docker stop ${args.container_name} && docker rm ${args.container_name}`;
                return handleShellCommand({ command: cmd }, context);
            }

            if (action === "logs") {
                if (!args.container_name) return { error: "Container name or ID is required to view logs." };
