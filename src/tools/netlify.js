...
export async function handleNetlifyTool(name, args, context) {
    if (name === "netlify_env_manage") {
        ...
        switch (args.action) {
            ...
            case "import":
                if (!args.file)
                    return { error: "File is required for 'import' action" };
    ...
    if (name === "netlify_functions_manage") {
        ...
        switch (args.action) {
            ...
            case "invoke":
                if (!args.name)
                    return { error: "Function name is required for 'invoke'" };
