...
const messageToApproval = new Map<string, string>();
...
export class ApprovalGate {
  ...
  async request(req: ApprovalRequest): Promise<ApprovalResult> {
    ...
    const channel = client.channels.cache.get(req.channelId) as TextChannel;
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: "⚡ Proposed Action", value: `\`\`\`${req.action}\`\`\`` },
        { name: "🎯 Risk Level", value: `**${req.riskLevel.toUpperCase()}** — ${risk.reason}`, inline: true },
        { name: "👤 Requested By", value: `<@${req.requestedBy}>`, inline: true },
  ...
  async handleReaction(
    ...
    const outcome: ApprovalOutcome = emoji === APPROVE_EMOJI ? "approved" : "rejected";
