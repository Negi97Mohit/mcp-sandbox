...
export interface AiVerificationReport {
    ...
    metrics: {
        schemaMatch: number;      // 0 or 1
        syntaxValid: number;      // 0 or 1
        safetyVerified: number;   // 0 or 1
        logicalConsistency: number; // 0 or 1
    };
    details: string[];
}
 ...
 */
export function checkTypescriptSyntax(code: string): { valid: boolean; error?: string } {
    try {
 ...
 */
export function checkSafety(text: string): { safe: boolean; error?: string } {
 ...
 */
export function validateAgentOutput(agentType: string, output: string, request: string): AiVerificationReport {
    ...
    if (agentType === "plan") {
        ...
        if (hasPlanMarker && hasConfidenceMarker && hasReasonMarker) {
            ...
            schemaMatch = 0;
            const missing = [];
            if (!hasPlanMarker) missing.push("PLAN:");
            if (!hasConfidenceMarker) missing.push("CONFIDENCE:");
            if (!hasReasonMarker) missing.push("REASON:");
        ...
        if (hasStatus && hasFiles) {
            ...
            schemaMatch = 0;
            const missing = [];
            if (!hasStatus) missing.push("IMPLEMENTATION_STATUS:");
            if (!hasFiles) missing.push("FILES_MODIFIED:");
            details.push(`❌ Schema mismatch: Missing markers [${missing.join(", ")}].`);
        }
        ...
        if (hasSummary && hasConf && hasCmd && hasTotal && hasPassing && hasFailing) {
            ...
            schemaMatch = 0;
            const missing = [];
            if (!hasSummary) missing.push("VERIFICATION_SUMMARY:");
            if (!hasConf) missing.push("CONFIDENCE:");
            if (!hasCmd) missing.push("TEST_COMMAND:");
            if (!hasTotal) missing.push("TOTAL_TESTS:");
            if (!hasPassing) missing.push("PASSING_TESTS:");
            if (!hasFailing) missing.push("FAILING_TESTS:");
            details.push(`❌ Schema mismatch: Missing markers [${missing.join(", ")}].`);
        }
    ...
    const score = Math.round(
        (schemaMatch * 0.3 + syntaxValid * 0.3 + safetyVerified * 0.2 + logicalConsistency * 0.2) * 100
    );
