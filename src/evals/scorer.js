 ...
 */
export function scoreToolAccuracy(actualTools, expectedTools) {
    if (expectedTools.length === 0)
        return 1.0; // No tool requirement → always passes
    const matched = expectedTools.filter((expected) => actualTools.some((actual) => actual === expected ||
        actual.startsWith(expected) ||
        expected.startsWith(actual)));
    return matched.length / expectedTools.length;
}
export function scoreOutputQuality(actualOutput, expectedContains) {
    if (!expectedContains || expectedContains.length === 0)
        return 1.0;
    const outputLower = actualOutput.toLowerCase();
    const matched = expectedContains.filter((phrase) => outputLower.includes(phrase.toLowerCase()));
    return matched.length / expectedContains.length;
}
export function computeOverallScore(toolScore, outputScore) {
    // Tools are weighted more heavily — calling the right tool is the core behavior
    return toolScore * 0.6 + outputScore * 0.4;
}
export function buildEvalResult(evalCase, actualTools, actualOutput, latencyMs, error) {
