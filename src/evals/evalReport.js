...
export function generateEvalReport(results) {
    ...
    const toolAccuracyAvg = results.reduce((sum, r) => sum + r.toolAccuracyScore, 0) / (results.length || 1);
    const outputQualityAvg = results.reduce((sum, r) => sum + r.outputQualityScore, 0) / (results.length || 1);
...
export async function saveEvalReport(report) {
...
export function printEvalSummary(report) {
    console.log("\n═══════════════════════════════════════════════");
...
export function formatDiscordReport(report) {
...
//# sourceMappingURL=evalReport.js.map