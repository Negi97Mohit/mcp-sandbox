...
client.on("messageReactionAdd", async (rawReaction: MessageReaction | any, rawUser: User | any) => {
    if (rawUser.bot) return;
    if (rawReaction.partial) {
        try { await rawReaction.fetch(); } catch { return; }
    }
    const emoji: string = rawReaction.emoji.name ?? "";
    if (emoji !== "✅" && emoji !== "❌") return;

    const isAdmin = permissionManager.isAdmin(rawUser.id);
    const handled = await approvalGate.handleReaction(
        rawReaction.message.id,
        emoji,
        rawUser.id,
        isAdmin
    );
...
async function handleAdminCommand(message: Message): Promise<void> {
    ...
    try {
        ...
        if (command === "!revoke") {
            const targetUser = message.mentions.users.first();
            if (!targetUser) { await message.reply("Usage: `!revoke @user`"); return; }
        ...
        if (command === "!permissions") {
            ...
            if (allUsers.length === 0) { await message.reply("No users have been granted access yet."); return; }
        ...
        if (command === "!workspace" && args[1] === "create") {
            const targetUser = message.mentions.users.first();
            if (!targetUser) { await message.reply("Usage: `!workspace create @user`"); return; }
        ...
        if (command === "!stats") {
            ...
            for (const chartEmbed of chartEmbeds) {
                if (message.channel.isSendable()) {
                    await (message.channel as TextChannel).send({ embeds: [chartEmbed] });
                }
            }
            return;
        }
        ...
        if (command === "!eval") {
            ...
            await message.reply(
                `🧪 Running eval regression suite${categoryFilter ? ` (category: ${categoryFilter})` : ""}...`
            );
            try {
 ...
 */
async function handleAgentCommand(message: Message): Promise<void> {
    ...
    try {
        ...
        switch (result.decision.action) {
            case "auto_commit": {
                ...
                summaryEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        { name: "📋 PR Title", value: d.prTitle },
                        { name: "🔬 Confidence", value: `${((result.verifyResult?.confidence ?? 0) * 100).toFixed(0)}%`, inline: true },
                        { name: "🧪 Tests", value: `${result.verifyResult?.testResults?.passing ?? "?"}/${result.verifyResult?.testResults?.totalTests ?? "?"} passing`, inline: true },
            ...
            case "request_approval": {
                ...
                summaryEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        { name: "📋 Reason", value: d.reason.substring(0, 500) },
                        { name: "📄 Diff Summary", value: `\`\`\`diff\n${d.diff.substring(0, 600)}\n\`\`\`` },
                        { name: "⏱ Time", value: `${totalSecs}s`, inline: true }
                    )
                    .setFooter({ text: `Task: ${result.taskId} | Trace: ${result.traceId ?? "local"}` });
                break;
            ...
            case "escalate": {
                ...
                summaryEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        { name: "📋 Reason", value: d.reason.substring(0, 600) },
                        { name: "⏱ Time", value: `${totalSecs}s`, inline: true }
                    )
                    .setFooter({ text: `Task: ${result.taskId} | Trace: ${result.traceId ?? "local"}` });
                break;
            ...
            default: {
                const d = result.decision as any;
                summaryEmbed = new EmbedBuilder()
        ...
        if (message.channel.isSendable()) {
            await (message.channel as TextChannel).send({ embeds: [summaryEmbed] });
        }
...
`,
    ...
    try {
        ...
        const histEntry: any = { role: "assistant", content: aiMessage.content, tool_calls: aiMessage.tool_calls };
        if (aiMessage.reasoning_details) histEntry.reasoning_details = aiMessage.reasoning_details;
        ...
        if (aiMessage.tool_calls) {
            for (const toolCall of aiMessage.tool_calls) {
                ...
                const sendLog = async (text: string) => {
                    const chunks = smartSplitMessage(text, 1900);
                    for (const chunk of chunks) {
                        if (message.channel.isSendable()) await (message.channel as any).send(chunk);
                    }
                };
            ...
            const finalEntry: any = { role: "assistant", content: finalMsg.content };
            if (finalMsg.reasoning_details) finalEntry.reasoning_details = finalMsg.reasoning_details;
...
async function sendStartupHealthReport(): Promise<void> {
    try {
