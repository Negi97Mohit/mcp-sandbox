 ...
 */
export function smartSplitMessage(text: string, maxLength: number = 2000): string[] {
    ...
    for (const line of lines) {
        ...
        let nextOpenCodeBlock: string | null = openCodeBlock;
        ...
        if (currentChunk.length + closingOverheadNow.length > 0) {
            // Close it
            if (openCodeBlock !== null) currentChunk += closingOverheadNow;
        ...
        if (freshLength <= maxLength) {
            ...
            while (remainingLine.length > 0) {
                ...
                if (remainingLine.length <= spaceLeft) {
                    ...
                    // Close and push
                    if (openCodeBlock !== null) currentChunk += "\n```";
                    chunks.push(currentChunk);

                    // Start next piece
                    currentChunk = "";
                    if (openCodeBlock !== null) currentChunk += `\`\`\`${openCodeBlock}\n`;
