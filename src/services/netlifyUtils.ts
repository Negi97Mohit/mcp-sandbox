 ...
 */
export function isSameRepo(url1: string | null | undefined, url2: string | null | undefined): boolean {
 ...
 */
export async function findNetlifySiteByRepo(repoUrl: string): Promise<any | null> {
 ...
 */
export async function getNetlifySite(siteIdOrName: string): Promise<any | null> {
     if (!CONFIG.NETLIFY_TOKEN) {
        throw new Error("NETLIFY_TOKEN is missing in the environment config.");
    }
    ...
    try {
        ...
        if (siteIdOrName.includes('.')) {
            ...
            return response.data.find((s: any) => s.name === siteIdOrName || s.default_domain === siteIdOrName) || null;
        } else {
             // Treat as ID
