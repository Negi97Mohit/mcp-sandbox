...
async function checkDeploymentsForSite(site: any) {
    ...
    let mostRecentCompletedDeploy = deploys.find((d: any) => d.state === 'ready' || d.state === 'error');

    if (!mostRecentCompletedDeploy) return; // All still building
...
async function reportDeployState(site: any, deploy: any) {
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(color)
            .addFields(
                { name: 'Commit', value: deploy.commit_ref ? `\`${deploy.commit_ref.substring(0, 7)}\`` : 'N/A', inline: true },
                { name: 'Branch', value: deploy.branch || 'main', inline: true },
                { name: 'Duration', value: duration, inline: true },
                { name: 'Message', value: commitMsg || 'No message' }
            )
            .setTimestamp()
        ...
        if (!isSuccess) {
            await performRCA(channel as TextChannel, site, deploy);
        }
...
async function performRCA(channel: TextChannel, site: any, deploy: any) {
    try {
        ...
        if (typeof logsRaw !== 'string') {
            ...
            if (Array.isArray(logsRaw)) {
              logsRaw = logsRaw.map((l: any) => l.log || "").join('\n');
