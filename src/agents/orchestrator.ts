...
export class Orchestrator {
  ...
  private implementer = new ImplementerAgent();
  ...
  async run(
    ...
    let verifyResult: AgentResult | undefined;
    ...
    try {
      ...
      graphStore.updateNode(planNode.id, {
        ...
        details: { ...planNode.details, plan: planResult.plan, error: planResult.error } as any
      });

      if (!planResult.success) {
        const result = this.buildResult(
          taskId, request, { action: "escalate", reason: `Planning failed: ${planResult.error}` },
          planResult, undefined, undefined, startTime, traceId
        );
      ...
      await context.sendLog(
        `✅ **Plan ready** | ${planResult.plan?.length ?? 0} steps | ` +
      ...
      if ((planResult.confidence ?? 0.5) < 0.3) {
        ...
        const result = this.buildResult(
          taskId, request,
          { action: "escalate", reason: `Planner confidence ${((planResult.confidence ?? 0) * 100).toFixed(0)}% is below safe threshold.\n\nPlanner output:\n${planResult.output.substring(0, 800)}` },
          planResult, undefined, undefined, startTime, traceId
        );
      ...
      const implSubTask: SubTask = {
        id: `${taskId}-impl`,
        type: "implement",
        description: `Execute this plan:\n${planResult.output}`,
        relatedFiles: planResult.plan?.map((s) => s.expectedOutput).filter(Boolean) as string[],
      ...
      graphStore.updateNode(implNode.id, {
        ...
        details: { ...implNode.details, modifiedFiles: implementResult.modifiedFiles, error: implementResult.error } as any
      });
      ...
      if (implementResult.output.includes("ESCALATE:")) {
        ...
        const result = this.buildResult(
          taskId, request,
          { action: "escalate", reason: escalateReason },
          planResult, implementResult, undefined, startTime, traceId
        );
      ...
      if (!implementResult.success) {
        const result = this.buildResult(
          taskId, request,
          { action: "escalate", reason: `Implementation failed: ${implementResult.error}` },
          planResult, implementResult, undefined, startTime, traceId
        );
      ...
      await context.sendLog(
        `✅ **Implementation done** | Files: ${implementResult.modifiedFiles?.join(", ") || "unknown"}`
      );
      ...
      graphStore.updateNode(verifyNode.id, {
        ...
        details: { ...verifyNode.details, testResults: verifyResult.testResults, confidence: verifyResult.confidence } as any
      });
      ...
      const result = this.buildResult(
        taskId, request, decision,
        planResult, implementResult, verifyResult,
        startTime, traceId
      );
      ...
      graphStore.addNode({
        ...
        details: { decision: decision.action, reason: (decision as any).reason, prTitle: (decision as any).prTitle }
      });
      ...
      return this.buildResult(
        taskId, request,
        { action: "escalate", reason: `Orchestrator error: ${error.message}` },
        planResult, implementResult, verifyResult,
        startTime, traceId
      );
    }
  ...
  private buildResult(
    ...
    if (planResult !== undefined)      result.planResult = planResult;
    if (implementResult !== undefined) result.implementResult = implementResult;
    if (verifyResult !== undefined)   result.verifyResult = verifyResult;
    if (traceId !== undefined)        result.traceId = traceId;
    return result;
  }

  /** Heuristic: extract likely relevant filenames from the request */
  private async discoverRelatedFiles(request: string, _context: AgentContext): Promise<string[]> {
    // Extract filenames mentioned in the request
    const fileMatches = request.match(/[\w/.-]+\.(ts|js|py|go|rs|json|yaml|yml|md)/g);
    return fileMatches ?? [];
  }

  private generatePRTitle(request: string): string {
...
`;
  ...
  private async getDiff(context: AgentContext): Promise<string> {
    try {
      const { executeToolCall } = await import("../tools/index.js");
      const result = await executeToolCall(
        "run_shell",
        { command: "git diff --stat HEAD" },
        { channelId: context.channelId, userId: context.userId, sendLog: context.sendLog }
      ) as any;
